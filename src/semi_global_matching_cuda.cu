...
__device__ __forceinline__ uint32_t ld_gbl_ca(const uint32_t* addr) {
	uint32_t return_value;
	asm("ld.global.ca.u32 %0, [%1];" : "=r"(return_value) : "l"(addr));
	return return_value;
}

__device__ __forceinline__ uint32_t ld_gbl_cs(const uint32_t* addr) {
	uint32_t return_value;
	asm("ld.global.cs.u32 %0, [%1];" : "=r"(return_value) : "l"(addr));
	return return_value;
}

__device__ __forceinline__ void st_gbl_wt(const uint32_t* addr, const uint32_t value) {
	asm("st.global.wt.u32 [%0], %1;" :: "l"(addr), "r"(value));
}

__device__ __forceinline__ void st_gbl_cs(const uint32_t* addr, const uint32_t value) {
...
__device__ __forceinline__ void CostAggregationDiagonalGeneric(uint8_t* d_cost, uint8_t* d_L, const int P1, const int P2, const int initial_row, const int initial_col, const int max_iter, const int col_nomin, const int col_copycost, const int cols, const T* _d_transform0, const T* _d_transform1, uint8_t* d_disparity, const uint8_t* d_L0, const uint8_t* d_L1, const uint8_t* d_L2, const uint8_t* d_L3, const uint8_t* d_L4, const uint8_t* d_L5, const uint8_t* d_L6, const int pathA) {
	const int lane = threadIdx.x % WARP_SIZE;
	const int dis = 4 * lane;
	int col = initial_col;
	int index = initial_row * cols * MAX_DISPARITY + initial_col * MAX_DISPARITY + dis;
...
__global__ void MedianFilter3x3(const uint8_t* d_input, uint8_t* d_out, const uint32_t rows, const uint32_t cols) {
